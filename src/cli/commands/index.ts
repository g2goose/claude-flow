          ...
          try {
            ...
            if (health.mcp) {
              ...
              if (mcpConfig && typeof mcpConfig === 'object') {
                const host = 'host' in mcpConfig ? mcpConfig.host : 'localhost';
                const port = 'port' in mcpConfig ? mcpConfig.port : 3000;
                const auth = 'auth' in mcpConfig ? mcpConfig.auth : false;
                console.log(`📍 Address: ${host}:${port}`);
                console.log(`🔐 Authentication: ${auth ? 'Enabled' : 'Disabled'}`);
                console.log(`🔧 Tools: Available`);
                console.log(`📊 Metrics: Collecting`);
              } else {
                console.log(`📍 MCP: Configuration not available`);
              }
            }
          } catch (err) {
