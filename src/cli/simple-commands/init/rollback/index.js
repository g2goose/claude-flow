...
export class RollbackSystem {
  constructor(workingDir) {
    ...
    this.incidentReporter = new IncidentReporter(workingDir);
  }
  ...
  /**
   * Create incident report for rollback operation
   */
  async createRollbackIncidentReport(rollbackData) {
    const result = {
      success: true,
      reportId: null,
      reportPath: null,
    ...
    try {
      console.log('📋 Creating rollback incident report...');

      const incidentData = {
        incidentType: rollbackData.type || 'Manual Rollback',
        severity: rollbackData.severity || 'Medium',
        status: rollbackData.status || 'Investigating',
        sessionId: rollbackData.sessionId || `rollback-${Date.now()}`,
        sourceCommit: rollbackData.sourceCommit || 'current',
        targetCommit: rollbackData.targetCommit || 'unknown',
        reason: rollbackData.reason || 'Rollback requested',
        userImpact: rollbackData.userImpact || 'Development environment only',
        duration: rollbackData.duration || 'Unknown',
        components: rollbackData.components || ['Claude Flow CLI'],
      };

      const incidentReport = await this.incidentReporter.generateIncidentReport(incidentData, {
        automated: rollbackData.automated || false,
        notes: rollbackData.notes || 'Rollback incident report generated by claude-flow system',
      });

      result.success = incidentReport.success;
      result.reportId = incidentReport.reportId;
      result.reportPath = incidentReport.reportPath;
      result.errors = incidentReport.errors;

      if (incidentReport.success) {
        printSuccess(`Incident report created: ${incidentReport.reportId}`);
        console.log(`  📄 Report location: ${incidentReport.reportPath}`);
      } else {
        printError('Failed to create incident report');
        result.errors.push(...incidentReport.errors);
      }
    } catch (error) {
      result.success = false;
      result.errors.push(`Failed to create incident report: ${error.message}`);
      printError(`Incident report error: ${error.message}`);
    }
  ...
  async listIncidentReports() {
    ...
    try {
      console.log('📋 Listing rollback incident reports...');

      const reports = await this.incidentReporter.listIncidentReports();
      result.success = reports.success;
      result.reports = reports.reports;
      result.errors = reports.errors;

      if (reports.success) {
        console.log(`📊 Found ${reports.reports.length} incident reports:`);
        reports.reports.forEach((report, index) => {
          console.log(`  ${index + 1}. ${report.id} (${report.severity}) - ${new Date(report.timestamp).toLocaleString()}`);
        });
      } else {
        printError('Failed to list incident reports');
      }
    } catch (error) {
      result.success = false;
      result.errors.push(`Failed to list incident reports: ${error.message}`);
      printError(`List reports error: ${error.message}`);
    }
